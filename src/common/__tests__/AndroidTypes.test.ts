...
describe('Android types tests', () => {
    ...
    test('Android Package class should correctly parse a raw string', async () => {
        let packages = AndroidPackage.parseRawString(AndroidMockData.mockRawPacakgesString);
        expect(packages !== null && packages.size == AndroidMockData.mockRawStringPackageLength);
    });

    test('Android Package class should correctly parse a raw string initialize members', async () => {
        let packages: Map<string, AndroidPackage> = AndroidPackage.parseRawString(AndroidMockData.mockRawPacakgesString);
        let pack: AndroidPackage| undefined = packages.get('build-tools;28.0.3');
        expect(packages.size == AndroidMockData.mockRawStringPackageLength &&  pack && pack.path !== null && pack.description !== null);
    });

    test('Android Package class should return and empty list for  a bad string', async () => {
        let packages = AndroidPackage.parseRawString(AndroidMockData.badMockRawPacakagesString);
